...
class Plotter(object):
  ...
  def plot_indexList(self, indexList, mode, title):
    ...
    for config_idx in indexList:
      print(f'[{self.exp}]: Plot {mode} results: {config_idx}')
      result = self.get_result(self.exp, config_idx, mode)
      if result is None:
        continue
      # Modify `hue_label` value in result for better visualization: add config index
      result[self.hue_label] = result[self.hue_label].map(lambda x: f'{x} {config_idx}')
      results = result if results is None else results.append(result, ignore_index=True)
  ...
  def plot_expIndexList(self, expIndexList, change_hue_label, title, image_name):
    '''
    Func: Given (exp, config index) list
    - merged == True: plot merged train result for all runs.
    - merged == False: plot unmerged train result of one single run.
    '''
    # Get results
    results = None
    for exp, config_idx, mode in expIndexList:
    ...
    # Plot
    if self.merged:
      image_path = f'./logs/{image_name}_merged.{self.imgType}'
    else:
      image_path = f'./logs/{image_name}.{self.imgType}'
    self.plot_vanilla(results, title, image_path)
