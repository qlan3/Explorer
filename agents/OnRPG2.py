    ...
    for _ in range(self.cfg['optimize_epochs']):
      ...
      for batch_idx in batch_idxs:
        ...
        if approx_kl <= 1.5 * self.cfg['target_kl']:
          ...
          ratio_v = torch.exp(prediction['log_pi'] - entries.log_pi[batch_idx]).detach()
          ratio_r = ratio_v.clone()
          mask1 = (entries.ppo_adv[batch_idx]>0) & (ratio_v > 1+self.cfg['clip_ratio'])
          mask2 = (entries.ppo_adv[batch_idx]<0) & (ratio_v < 1-self.cfg['clip_ratio'])
          ratio_v[mask1 | mask2] = 0.0
          # obj = predicted_reward + entries.adv[batch_idx] * prediction['log_pi']
          # actor_loss = -(ratio_v*obj).mean()
          obj = entries.adv[batch_idx] * prediction['log_pi']
          ratio_r[mask1] = 1+self.cfg['clip_ratio']
          ratio_r[mask2] = 1-self.cfg['clip_ratio']
          actor_loss = -(ratio_r*predicted_reward + ratio_v*obj).mean()
