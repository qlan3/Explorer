...
class OnRPG(SAC):
  ...
  def __init__(self, cfg):
    ...
    self.storage = InfiniteReplay(keys=['state', 'action', 'reward'])
  
  def reset_game(self, mode):
    super().reset_game(mode)
  ...
  def learn(self):
    ...
    if (self.step_count // self.cfg['network_update_frequency']) % self.cfg['actor_update_frequency'] == 0:
      ...
      entries = self.storage.get(['state', 'action'], self.episode_step_count[mode])
      # Compute actor loss
      actor_loss = self.compute_actor_loss(entries)
